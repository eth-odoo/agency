...
class PortalMain(http.Controller):
    ...
    @http.route('/agency/login', type='http', auth='public', website=True)
    ...
    @http.route('/agency/login', type='http', auth='public', website=True, methods=['POST'], csrf=True)
    def portal_login_submit(self, **kwargs):
        ...
        if result.get('success'):
            # Store session data
            request.session['agency_token'] = result.get('token')
            request.session['agency_user_id'] = result.get('user_id')
            request.session['agency_id'] = result.get('user_data', {}).get('agency_id')

            _logger.info(f"Agency user logged in: {email}")
            return request.redirect('/agency/dashboard')
        else:
            _logger.warning(f"Failed login attempt: {email}")
    ...
    @http.route('/agency/forgot-password', type='http', auth='public', website=True)
    ...
    @http.route('/agency/forgot-password', type='http', auth='public', website=True, methods=['POST'], csrf=True)
    ...
    @http.route('/agency/reset-password/<string:token>', type='http', auth='public', website=True)
    ...
    @http.route('/agency/reset-password/<string:token>', type='http', auth='public', website=True, methods=['POST'], csrf=True)
